...
func init() {
	...
	f.PersistentFlags().String("bridgeaddress string", "", "Bridge smart contract address")
	f.PersistentFlags().String("wzcnaddress string", "", "WZCN token address")
	f.PersistentFlags().String("authorizersaddress string", "", "Authorizers smart contract address")
	f.PersistentFlags().String("ethereumnodeurl string", "", "Ethereum Node URL (Infura/Alchemy)")
	f.PersistentFlags().Int64("gaslimit int", 0, "Appr. gas limit to execute Ethereum transaction")
