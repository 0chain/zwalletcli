...
func (zcn *ZCNStatus) OnWalletCreateComplete(status int, wallet string, err string) {
  ...
  if status != zcncore.StatusSuccess {
    zcn.success = false
    zcn.errMsg = err
    fmt.Println("aa3", zcn.errMsg)
    zcn.walletString = ""
    return
  }
  zcn.success = true
  zcn.errMsg = ""
  fmt.Println("aa4", zcn.errMsg)
...
func (zcn *ZCNStatus) OnInfoAvailable(Op int, status int, config string, err string) {
  ...
  if status != zcncore.StatusSuccess {
    zcn.success = false
    zcn.errMsg = err
    fmt.Println("aa5", zcn.errMsg)
    return
  }
  zcn.success = true
  zcn.errMsg = config
  fmt.Println("aa6", zcn.errMsg)
}
...
func (zcn *ZCNStatus) OnVoteComplete(status int, proposal string, err string) {
  ...
  if status != zcncore.StatusSuccess {
    zcn.success = false
    zcn.errMsg = err
    fmt.Println("aa7", zcn.errMsg)
    zcn.walletString = ""
    return
  }
  zcn.success = true
  zcn.errMsg = ""
  fmt.Println("aa8", zcn.errMsg)
