...
func init() {
	...
	minerscMiners.PersistentFlags().Bool("json boolean", false, "as JSON")
	minerscMiners.PersistentFlags().Int("limit int", 20, "Limits the amount of miners returned")
	minerscMiners.PersistentFlags().Int("offset int", 0, "Skips the number of miners mentioned")
	minerscMiners.PersistentFlags().Bool("active boolean", false, "Gets all miners, including inactive miners")
	minerscMiners.PersistentFlags().Bool("all boolean", false, "include all registered miners")
	minerscSharders.PersistentFlags().Bool("json boolean", false, "as JSON")
	minerscSharders.PersistentFlags().Int("limit int", 20, "Limits the amount of sharders returned")
	minerscSharders.PersistentFlags().Int("offset int", 0, "Skips the number of sharders mentioned")
	minerscSharders.PersistentFlags().Bool("all boolean", false, "include all registered sharders")
	minerscSharders.PersistentFlags().Bool("active boolean", false, "Gets all sharders, including inactive sharders")

	minerscInfo.PersistentFlags().String("id string", "", "miner/sharder ID to get info for")
	minerscInfo.MarkFlagRequired("id")

	minerscUserInfo.PersistentFlags().String("client_id string", "", "get info for user, if empty, current user used, optional")
	minerscUserInfo.PersistentFlags().Bool("json boolean", false, "as JSON")

	minerscPoolInfo.PersistentFlags().String("id string", "", "miner/sharder ID to get info for")
	minerscPoolInfo.MarkFlagRequired("id")

	minerscLock.PersistentFlags().String("miner_id string", "", "miner ID to lock stake for")
	minerscLock.PersistentFlags().String("sharder_id string", "", "sharder ID to lock stake for")
	minerscLock.PersistentFlags().Float64("tokens float", 0, "tokens to lock")
	minerscLock.MarkFlagRequired("tokens")

	minerscUnlock.PersistentFlags().String("miner_id string", "", "miner ID to lock stake for")
	minerscUnlock.PersistentFlags().String("sharder_id string", "", "sharder ID to lock stake for")
}
