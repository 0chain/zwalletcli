...
setup() {
    ...
    if [ -f /etc/apt/sources.list.d/zwallet.list ]; then
      rm /etc/apt/sources.list.d/zwallet.list
    fi
...
Signed-by: /etc/apt/keyrings/zus.gpg" | tee /etc/apt/sources.list.d/zwallet.sources
    apt-get update
    set +v

    apt-get install --assume-yes zwallet
